...
class Ground(engine.GameObject):
    def __init__(self, compounds=()):
        ...
        super().__init__(0, 0, 0, 0, 'ground', 'black', True)  # True because static objects

    @staticmethod
    def init_ground(level="lvl1"):
        assert game.Game.ground is None, "Ground already initialized"
        print("Initializing the ground...")
        path = "Files/lvls/" + level + "/ground.txt"
        with open(path, 'r') as f:
            lines = f.readlines()
            assert len(lines) % 2 == 0, "Inconsistent file: " + path
            for i in range(len(lines) // 2):
                points = lines[2 * i].split()
                pos = lines[2 * i + 1].split()
                posi, posj = int(pos[0]), int(pos[1])
                poly = []
                assert len(points) % 2 == 0, "Inconsistent file: " + path
    ...
    def init_doors(level="lvl1"):
        ...
        with open(path, 'r') as f:
            ...
            for line in lines[1:]:
                ...
                posi, posj = int(line_split[0]), int(line_split[1])
                x, y = int(line_split[2]), int(line_split[3])
                color = line_split[4]
                angle = int(line_split[5])
                keys_required = line_split[6].split(',')
    ...
    def init_keys(level="lvl1"):
        ...
        with open(path, 'r') as f:
            ...
            for line in lines[1:]:
                ...
                x, y = int(line_split[2]), int(line_split[3])
                color = line_split[4]
                key_id = int(line_split[5])
                newi, newj = int(line_split[6]), int(line_split[7])
                newx, newy = int(line_split[8]), int(line_split[9])
...
def collide_door(roundobj):
    ...
    for door in Door.ldoor[game.Game.posi][game.Game.posj]:
        if door.x == 0 and abs(door.y - roundobj.y) < roundobj.radius + 40 and \
                abs(door.x - roundobj.x) < 100 + roundobj.radius:  # 40 = half width of the door, 100 = half height
            return True
        elif door.y == 0 and abs(door.x - roundobj.x) < roundobj.radius + 40 and \
                abs(door.y - roundobj.y) < 100 + roundobj.radius:
            return True
    return False
