...
class Bullet(engine.GameObject):
    ...
    def fire_at_rocket(enemy):
        ...
        if enemy.x == game.Game.rocket.x:
            ...
            num = enemy.y - game.Game.rocket.y
            denum = enemy.x - game.Game.rocket.x
            angle = angle_to_be_added + math.degrees(math.atan(num / denum))
        engine.add_obj(Bullet(enemy.x, enemy.y, angle, False))

    @staticmethod
    def collision_point(x, y, angle):
        """Computing the collision point of the bullet assuming that it goes in a straight line"""
                    ...
                    if newdist < dist_collision:  # Closer than the last point
                        ...
                        if (angle_360 <= 180 and newy <= y) or \
                            ...
                            if ((angle_360 <= 90 or angle_360 >= 270) and newx <= x) \
                                ...
                                # checking if the point is on the segment
                                if min(x1, x2) <= newx <= max(x1, x2) and min(y1, y2) <= newy <= max(y1, y2):
