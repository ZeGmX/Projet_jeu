...
class Menu:
    ...
    # may be different of cursor_position if there are more items thant what can be displayed on one page
    skin_cursor = 0
    ...
    def keyboard_skin(key):
        ...
        if key == "Return":
            Rocket.skin = Menu.skin_list[Menu.skin_cursor]
            Rocket.radius = Menu.skin_radius_list[Menu.skin_cursor]
            engine.del_obj(Menu.current_skin)
            engine.del_obj(Menu.current_skin_powered)
            engine.del_obj(Menu.skin_rect)
            Menu.current_skin = Menu.current_skin_powered = Menu.skin_rect = None
            Menu.load_main_menu()
